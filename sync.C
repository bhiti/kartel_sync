...
void synchronize(const char* fname_tel="", const char* fname_anchor="", const char* fname_synced="", int Nevents = 123456789)
{
...
//   int Nevents = 200000;     // EDIT HERE: number of events to process
  ...
  int sync_mode=1;    // 0... sync DRS+tel, 1...sync FEI4+tel
...
//   }
  ...
  run *r = new run("", fname_tel, fname_anchor, fname_synced);
...
//   r->rel_offset = 214;
  ...
  cout << "Event \trel. \tabs. \tpassed" << endl;
  for (i=istart; i<iend; i++) 
  {
    ...
    if (i%10000 == 0){
      cout << i << "\t" << r->rel_offset << "\t" << r->abs_offset << "\t" << passed << endl;
//       cout << r->IOHand->outputEvents->GetEntries() << " " << i << endl;
    }

    
//     if (i>151000) break;
    //  do manual correction here if required
//     if (strncmp(run_number, "4261",4 )==0) {
//       if (i==1480000){
//         int inew=150000;      // to which event to skip
//         for (int j=i; j<inew; j++) r->FillSyncedFileDummy(j);
//         i=inew;
//         r->rel_offset = 219;
//         cout << "SKIPPING EVENTS MANUALLY" << endl;
//         continue;
//       }
//     }
//     
