{
  //TH1S *hC = new TH1S("hC", "Collected charge; U_{CSA} (mV) ; N_{entries}", 300,-0.02,0.2);
//   TH1I *hC = new TH1I("hC", " ; U_{CSA} (V) ; N_{entries}", 200,-0.02,0.3);
  ...
  char* run_number="097";
  TString plotsDir = Form("plots/run%s-asd", run_number);
  gSystem->MakeDirectory(plotsDir.Data());
  int ch[NCH_DRS]={0,1,2,-3,-4,-1,-1,-1};   // ch/drs_input: 0/1, 1/2, 2/3, 3/4
  int charge_mode=1;    // 0 ... current sensitive amp->waveforms will be integrated, 1 ... charge sensitive amplifier -> find steps in waveforms   
  int pulse_polarity = 1;   // EDIT HERE: positive or negative signals
  int Nevents = 330000;     // EDIT HERE: number of events to process
  
  bool using_anchor = true;
//   bool using_anchor = false;
  
  float zDut[NCH_DRS] = {120000.0, 120000.0, 0.}; // DUT position along z-axis in micrometers (from 1st mimosa plane)
  float zAnchor=260000;
  int verbose=0;
...
//   else run *r = new run(fname_drs, fname_tel, fname_anchor, fname_res, brname_drs, brname_drs, v_lname_drs);
  ...
  for (int i=0; i<NCH_DRS; i++) r->zDUT[i] = zDut[i];
  r->zAnchor = zAnchor;
...
//   fiducial_region[0].Set(-600,350, 1100, 2050); fiducial_region[1].Set(-600,350, 1100, 2050); // run 087
  fiducial_region[0].Set(-1375,-1330, 2510, 2555); fiducial_region[1].Set(-1425,-1380, 2510, 2555); // run 097
...
//       r->SetNoiseWindow(i,100., 200.);
      r->SetSignalWindow(i,360, 450);
      r->SetNoiseWindow(i,100., 190.);
    }
  }
  for (int i=0; i<r->nch; i++) 
  {
...
//       gPad->Update();
      ...
      c2->cd();
      r->Event.Draw(1);
      r->Event.gr[1]->GetXaxis()->SetRangeUser(300,600);
      r->Event.gr[1]->GetYaxis()->SetRangeUser(-0.03,0.1);
