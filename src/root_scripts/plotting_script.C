...
//   {
//     hs[i] = (TH1I*) hC[i]->Clone(Form("hs[%d]",i));
//     DrawTH1 (hs[i]);
//   }
//   f.Write();
//   f.Close();
...
//     fgaus[i] = new TF1(Form("gaus%d",i),"gaus", -0.5*thr[i], 0.5*thr[i]);
    hC[i]->Fit(Form("gaus%d",i),"RQ");
    flangau[i] = new TF1(Form("langau%d",i),langaufun, 1.8*thr[i], 0.3, 4);
    flangau[i]->SetParameters(5e-3 , 0.05, 10, 5e-2);  // scale, MPV, intgral, gaus_sigma
    flangau[i]->FixParameter(3,fgaus[i]->GetParameter(2));
    hC[i]->Fit(Form("langau%d",i),"RQ");
  //   hC->GetXaxis()->SetRangeUser(-0.01,0.04);
    hC[i]->Draw();
    fgaus[i]->Draw("SAME");
    TLine tl(thr[i], 0, thr[i], hC[i]->GetMaximum());
...
//     gPad->SetLogy(0);
    ...
    hN[i]->Fit(Form("noise%d",i),"RQ");
  //   hC->GetXaxis()->SetRangeUser(-0.01,0.04);
    hN[i]->Draw();
    tl.Draw();
//     tex.DrawLatexNDC(0.72,0.55, Form("#splitline{Noise = %.1lf mV}{Thr = %.1lf mV}", 1000*hN[i]->GetRMS(), thr[i]*1000)); 
...
//     hC[i]->GetYaxis()->SetRangeUser(0, 3*hC[i]->GetBinContent( hC[i]->FindBin(3*thr[i])));
    hSub->Draw();
    ...
    tex.DrawLatexNDC(0.7,0.35, Form("#splitline{#splitline{#splitline{Langau fit:}{MPV = %.0lf mV}}{#sigma_{gaus} = %.1lf mV}}{#Chi^2/NDF = %.1lf}", 1000*flangau[i]->GetParameter(1), 1000*flangau[i]->GetParameter(3), flangau[i]->GetChisquare()/flangau[i]->GetNDF())); 
// 
//     tex.DrawLatexNDC(0.33,0.85, Form("#splitline{Pedestal (#sigma) = %.1lf mV}{Thr = %.1lf mV}", 1000*fgaus[i]->GetParameter(2), thr[i]*1000)); 
//         tex.DrawLatexNDC(0.3,0.65, Form("#splitline{Noise = %.1lf mV}{Thr = %.0lf mV}", 1000*fgaus[0]->GetParameter(2),thr[0]*1000)); 
//     tex.DrawLatexNDC(0.7,0.35, Form("#splitline{#splitline{#splitline{Langau fit:}{MPV = %.0lf mV}}{#sigma_{gaus} = %.0lf mV}}{#Chi^2/NDF = %.1lf}", 1000*flangau[i]->GetParameter(1), 1000*flangau[i]->GetParameter(3), flangau[i]->GetChisquare()/flangau[0]->GetNDF())); 
...
//   r->hTracks_ROI->Draw("COLZ");
  ...
  TGraph gavgwf(1000, t, r->avwf.wf);
  gPad->SetLogy(0);
  gavgwf.Draw("APL");
  */
...
//   TCanvas *c1 = new TCanvas("c1", "c1", 1);
//   TH2S h_qt("h_qt", "Charge vs. t_{peak}", 50,0,1, 100,0,DRS_N_POINTS);
//   for(int ipt=0; ipt<r->N; ipt++) h_qt.Fill(r->v_max[ipt], r->v_posmax[ipt]); 
//   h_qt.Draw("colz");
//   
...
//   DrawTH2(r->heff_tracks[0], "", 1.1, 1.1, 1);
  DrawTH2(r->heff_tracks[0],"", 1.05, 1.2, 1.05);
  r->heff_tracks[0]->Draw("COLZ");
  ...
  {
    for (int ix=1; ix<= r->heff_efficiency[ich]->GetNbinsX(); ix++)
      for (int iy=1; iy<= r->heff_efficiency[ich]->GetNbinsY(); iy++)
      {
//         int ntr=TMath::Max(r->heff_tracks[ich]->GetBinContent(ix,iy),1.0);
        int ntr=r->heff_tracks[ich]->GetBinContent(ix,iy);
        if (ntr==0) continue;
        float eff = 1.0*r->heff_hits[ich]->GetBinContent(ix,iy) / ntr;
        if (eff==0) eff=0.001;   // distinguish non efficient bins from empty ones
        r->heff_efficiency[ich]->SetBinContent(ix,iy,eff);

        double chrg = r->heff_charge[ich]->GetBinContent(ix,iy) / ntr;
        r->heff_charge[ich]->SetBinContent(ix,iy, chrg); 
        r->hpulse_height[ich]->Fill(chrg);
      }
  
    DrawTH2(r->heff_efficiency[ich],"", 1.05, 1.2, 1.05);
    r->heff_efficiency[ich]->GetZaxis()->SetRangeUser(0,1);
    r->heff_efficiency[ich]->Draw("COLZ");
    c3->Print(Form("%s/eff_ch%d.pdf",plotsDir.Data(), ich+1));
    c3->Print(Form("%s/eff_ch%d.png",plotsDir.Data(), ich+1));
    
    DrawTH2(r->heff_charge[ich],"", 1.05, 1.2, 1.05);
    r->heff_charge[ich]->GetZaxis()->SetRangeUser(TMath::Min(0., r->heff_charge[ich]->GetMinimum()), r->heff_charge[ich]->GetMaximum());
    r->heff_charge[ich]->Draw("COLZ");
    c3->Print(Form("%s/ph_ch%d.pdf",plotsDir.Data(), ich+1));
    c3->Print(Form("%s/ph_ch%d.png",plotsDir.Data(), ich+1));
    
    DrawTH1(r->hpulse_height[ich],"", 1.05, 1.2);
    r->hpulse_height[ich]->GetYaxis()->SetRangeUser(0, 2*r->hpulse_height[ich]->GetBinContent(r->hpulse_height[ich]->FindBin(50)));
    r->hpulse_height[ich]->Draw("COLZ");
  ...
  for (int i=0; i< r->nch; i++)
  {
    ...
    for (int ix=1; ix<=r->heff_efficiency[i]->GetNbinsX(); ix++)
      for (int iy=1; iy<=r->heff_efficiency[i]->GetNbinsY(); iy++)
      {
        float xc = r->heff_efficiency[i]->GetXaxis()->GetBinCenter( ix );
        float yc = r->heff_efficiency[i]->GetYaxis()->GetBinCenter( iy );
        if (! fiducial_region[i].Contains(xc, yc)) continue;
        if (r->heff_tracks[i]->GetBinContent(ix,iy)) heff_distribution[i]->Fill( r->heff_efficiency[i]->GetBinContent(ix,iy));  
      }
